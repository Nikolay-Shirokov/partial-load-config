...
# С нестандартным каталогом конфигурации
.\partial-load-config.ps1 -CommitId "feature/new-report" `
...
| `V8Path` / `/v8` | `1cv8.exe` | Путь к исполняемому файлу платформы |
| `OutFile` / `/out` | temp файл | Файл для вывода служебных сообщений |
| `Debug` / `/debug` | - | Включить режим отладки |
...
2. **Фильтрация файлов**
   ...
   - Проверяется расширение файла (`.xml`)
...
При возникновении проблем используйте параметр `-Debug`:

```powershell
.\partial-load-config.ps1 -CommitId "HEAD" -InfoBasePath "C:\Bases\Test" -Debug
```
...
1. Работает только с файлами формата XML (не с `.cf`, `.cfu`)
2. Требует предварительную выгрузку конфигурации в файлы
3. Не поддерживает расширения конфигурации
4. Загружает только измененные файлы (не обрабатывает удаленные)
...
### Ошибка: "Не найдено файлов для загрузки"
- Проверьте идентификатор коммита
- Убедитесь, что в коммите есть изменения XML файлов
- Проверьте путь к каталогу конфигурации
